...
| 9 | 무너진 수도 | Royal Cavalry counterplay reinforcement | 900 | 900 gold and unlock integrated Hero Hall mastery training |
...
- Korean/English/Japanese localization infrastructure is **Implemented** across the title/archive, opening, map hub, progression screens, battle HUD, Phaser-authored battle labels, results, canonical combatant names, campaign/challenge names, achievements, and accessibility labels. English has the broadest translated coverage; Japanese currently covers the primary navigation, map, progression, battle, roster, hero, and opening vocabulary and is therefore **Partial** for remaining long-form lore and secondary copy. A missing Japanese entry falls back to English, then to the canonical Korean source key, so the interface never becomes blank. On the first visit, `ko-*` locales select Korean, `ja-*` locales select Japanese, and all others select English. A single registry-driven language dropdown appears on the title and in-game shell header, and the global `last-bastion-language` localStorage preference remains independent from all three campaign slots, portable saves, and save-schema migration. Switching language updates `<html lang>`, localized save timestamps, the document title, and its description. Non-Korean resources are dynamically imported.
...
- The kingdom map is the persistent in-game hub. Its `왕국 운영` panel opens the armory, integrated hero hall, fortress technology, stage-30 Victory Monument, achievements, war codex, daily reward, stage-unlocked mysterious merchant, and global sound setting. Hero mastery training is not a separate map operation. Beast challenges exist only as map nodes. On desktop this compact 176 px vertical panel occupies a dedicated left column outside the heading, stage map, and mission panel. At 820 px and below it becomes a compact toolbar above the map content, using four columns on tablet and two on phone; it never overlays or occupies the map canvas. Main labels remain at least 12–13 px and metadata at least 9 px even as button height is reduced.
- First-clear rewards can unlock features through structured `featureId` metadata. `hero-training` is unlocked by the persisted stage-9 clear rather than a redundant save flag, so old profiles that already cleared stage 9 receive it automatically. Before that milestone every recruited Hero Hall card shows the locked training area and its chapter-9 requirement; there is no separate disabled operations button.
- Integrated Hero Hall training is the first repeatable late-game Gold sink. It offers 100 XP for 250 Gold, 500 XP for 1,000 Gold, and 1,500 XP for 2,500 Gold directly on each recruited hero card, beside that hero's recruitment, selection, equipment, mastery, awakening, aura, and skill information. The store enforces the stage-9 gate, ownership, affordability, and the level-30 total-XP cap; battle usage continues to grant XP independently. The persistent feature ID remains `hero-training` for save compatibility even though the standalone screen and route have been removed.
...
- The language control is a custom dark-glass dropdown generated from `supportedLanguages`, allowing another locale to be added without duplicating controls. Its icon-free trigger shows the current native language name and code, while its menu shows language-code badges, native names, hover/focus treatment, and a selected check mark. It supports outside-click dismissal, Escape with trigger-focus restoration, Tab dismissal, and Arrow/Home/End option navigation. The title places the full trigger in the top navigation; phone-width progression headers collapse it to the current `KO`/`EN`/`JA` code without hiding its accessible label, and the menu still exposes full names. React screen roots pass through `Localized`, and Phaser-created labels call the same `t` function directly, so both rendering owners share one language preference without coupling it to progression state.
- Armory troop cards use a compact two-column presentation: canonical character art has no decorative diamond behind it, the troop name and fixed star grade share one header row on desktop, and primary text retains its readable size while padding and secondary gaps are reduced. Narrow screens may stack the title and grade to prevent truncation. Interactive buttons expose consistent hover brightness and pressed feedback; armory actions additionally move by one pixel to reinforce pointer input. Reduced-motion preferences disable nonessential transitions.
- Desktop Hero Hall cards stretch their information column to the shared grid-row height and anchor statistics, equipment, selection, and integrated mastery training to the bottom. Heroes with an additional guard-trait panel therefore keep their lower controls aligned with heroes that have the standard three trait panels; tablet and phone layouts continue to flow naturally as single-column content.
- Transient notices use a centered, bounded dark glass toast with a gold state marker, strong contrast, safe-area-aware placement, and a short entrance/exit animation instead of the former flat light rectangle. Reduced-motion preferences render it without animation.
...
- Localization: project-local Korean-source-key translator with dynamically imported `ko`/`en`/`ja` message modules, a registry-driven locale selector, and a small Zustand notification store; no third-party i18n runtime
...
- `src/shared/i18n/ja/messages.ts`: Japanese overrides loaded over the English fallback resource
...
- `src/data/features.ts`: stage-unlocked features and integrated Hero Hall training-package data
...
- 2026-09-19: Integrated stage-9 Gold-funded hero mastery training into recruited Hero Hall cards, exposed the locked requirement in place before chapter 9, removed the redundant map operation and standalone screen/route, renamed the reward to the returning royal instructors, and preserved the existing store gate and save-compatible `hero-training` feature ID.
- 2026-09-19: Compacted armory troop cards by removing the portrait diamond, sharing one desktop header row between name and fixed grade, and tightening non-text spacing; added consistent hover/pressed button feedback and redesigned transient notices as animated, safe-area-aware dark glass toasts.
- 2026-09-19: Replaced the fixed KO/EN button pair with a registry-driven language dropdown, added browser-detected Japanese with localized timestamps and broad navigation/map/progression/battle/roster vocabulary, preserved lazy resource loading, and established English-then-Korean fallback for missing Japanese copy.
