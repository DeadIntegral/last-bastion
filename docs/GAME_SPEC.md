...
- Legendary 4-star and transcendent 5-star troops retain a minimum four-digit base-health identity. Current 4-star examples range from the 1,200-HP Reaper to the 2,300-HP Rune Golem, while the post-finale 5-star Ifrit has 11,000 base HP. Ifrit is absent from campaign waves and reinforcements and first appears only in challenge 104 after clearing stage 30. Its larger body is constrained by a full 200 Command cost, one-body deployment, a 6.6-second cooldown, and a two-body living cap.
...
- The player persists a battle formation containing one to four acquired troop types by default. The permanent `formationSlotUnlocked` entitlement raises this to five; battle cards and numeric hotkeys follow formation order, including key `5` for the expanded slot.
...
- New saves separately store gold, Royal Gems, the last daily claim date, battle-speed entitlement and preference, the formation-slot entitlement, per-slot equipment levels for all shared troops, acquired troops, the one-to-four-or-five troop battle formation, cleared stages, mastery XP, recruited heroes, fortress research, career statistics, encountered troops and boss, unlocked achievements, and claimed rewards.
...
- Royal Gems start at zero. The kingdom map grants 10 once per browser-local calendar date and its sticky header shows both wallet currencies. Clearing stage 6 reveals the `수수께끼 상인` operation and its permanent 200-Gem battle-speed license. Clearing stage 12 reveals the permanent 150-Gem `편성 확장 허가`, which raises formation capacity from four to five. Both purchases are idempotent, enforced in the store, and persisted across local slots and portable exports. There is no recharge or real-money purchase mechanism. A future server-verified Quick Starter may grant some Royal Gems plus the same `battleSpeedUnlocked` and `formationSlotUnlocked` entitlements without their stage gates; client-only payment inference remains forbidden and the payment bundle is still Planned.
...
- `1`–`4`: summon the corresponding troop in the default formation; `5` summons the expanded slot after obtaining `편성 확장 허가`.
